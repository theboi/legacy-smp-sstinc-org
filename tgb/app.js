...
bot.command("info", (ctx) => {
  ctx.reply(`Developed by <b>Granwyn Tan</b>, SST Inc., 2021`, {
    parse_mode: "HTML",
  });
});
      ...
      bot.telegram
        ...
        .then(function (data) {
          ...
          if (ctx.from._is_in_admin_list) {
            ctx.reply(
              `${third} points ${second}ed for @${ctx.message.from.username}`,
              {
                reply_to_message_id: ctx.message.message_id,
                parse_mode: "HTML",
              }
            );
            ctx.reply(`@${ctx.message.from.username} now has X points`, {
              parse_mode: "HTML",
            });
          } else {
            ctx.reply(
              `<b>${ctx.message.from.first_name} ${ctx.message.from.last_name}</b> @${ctx.message.from.username} you do not have permissions to give, deduct or manage points`,
              {
                reply_to_message_id: ctx.message.message_id,
                parse_mode: "HTML",
              }
...
bot.on("inline_query", async (ctx) => {
  ...
  result = resultsArray.map((elem, index) => {
    ...
    return {
      ...
      id: String(index),
      title: elem.title,
      description: `by ${elem.owner.display_name}`,
      input_message_content: {
        message_text: `Title: ${elem.title}\nby ${elem.owner.display_name}\nTags: ${elem.tags}\n${elem.link}`,
        url: elem.link,
      },
    };
  });
